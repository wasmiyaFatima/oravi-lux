...
export function Values() {
  ...
  return (
    <section className="relative overflow-hidden border-t border-border bg-dark py-24 text-on-dark md:py-32">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_70%_55%_at_0%_0%,rgba(243,217,160,0.22),transparent_55%)]" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_50%_40%_at_100%_100%,rgba(243,217,160,0.12),transparent_50%)]" />
      ...
      <Container className="relative">
        <div className="grid gap-14 lg:grid-cols-12 lg:items-start lg:gap-20">
          <Reveal className="lg:col-span-5 lg:sticky lg:top-28" variant="fade-right">
            ...
            <p className="mt-6 font-display text-xl text-accent-on-dark md:text-2xl">
              {t("tagline")}
            </p>
            <div className="mt-10">
            ...
            {items.map((item, index) => (
              <Reveal key={item.title} delay={index * 0.06} variant="fade-left">
                <li className="group flex items-baseline gap-6 border-b border-white/10 py-6 first:pt-0 last:border-b-0 md:gap-10 md:py-7">
                  <span className="font-display w-10 shrink-0 text-xl text-accent-on-dark/50 transition-colors group-hover:text-accent-on-dark md:w-12 md:text-2xl">
